...
import torch
from .fca_layer import MultiSpectralAttentionLayer  # fca, xyl20220320
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1,
                 downsample=None, dilation=1, reduction=16):
        ...
        self.relu = nn.ReLU(inplace=True)
        c2wh = dict([(64, 63), (128, 31), (256, 31), (512, 31)])  # c2wh = dict([(64,56), (128,28), (256,14) ,(512,7)]) # RuntimeError: The size of tensor a (28) must match the size of tensor b (15) at non-singleton dimension 3
        # channel to img w/h
        self.att = MultiSpectralAttentionLayer(planes * 4, c2wh[planes], c2wh[planes], reduction=reduction,
                                               freq_sel_method='top16')  # fca, xyl20220320.
    ...
    def forward(self, x):
        ...
        out = self.conv3(out)
        out = self.bn3(out)
        out = self.att(out)  # fca, xyl20220320
